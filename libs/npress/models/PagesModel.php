...
class PagesModel extends Object
{
  ...
  public static function getChildNodesUnfiltered($id, $lang = false)
  {
    ...
    if ($pages) {
      $metas = dibi::query(
        ...
        array_keys($pages),
        "
				ORDER BY id_page,`key`"
      )->fetchAssoc('id_page|key=value');
    }
  ...
  public static function getChildNodes($id, $lang = false)
  {
  ...
  public static function getChildNodesWithMeta($id, $lang, $key, $value)
  {
    //used by Node methods
  ...
  public static function getPageBySeoname($seoname, $lang = false)
  {
    ...
    $page = dibi::fetch(
      "SELECT * FROM pages WHERE lang = %s",
      $lang,
      " AND seoname = %s",
      $seoname,
      " AND deleted=0"
  ...
  public static function cacheAllPages()
  {
    ...
    $metas = dibi::query(
      "SELECT * FROM pages_meta ORDER BY id_page,`key`"
    )->fetchAssoc('id_page|key=value');
    ...
    foreach ($pages as $id_page => $data) {
      ...
      if (!isset(self::$cacheByParent[$obj->lang][$obj->id])) {
        //leafs childs
  ...
  public static function pagesModelNode($data, $meta = null)
  {
    ...
    if (!isset($meta)) {
      $meta = (array) dibi::query(
        "SELECT * FROM pages_meta WHERE id_page = %i",
        $data['id_page'],
        " ORDER BY `key` "
  ...
  public static function addPage($data)
  {
    ...
    if (!isset($data['ord'])) {
      $data['ord'] =
        1 +
        dibi::fetchSingle(
          'SELECT max(ord) FROM pages WHERE lang=%s',
          $data['lang'],
          " AND id_parent=%s",
          $data['id_parent']
        );
...
class PagesModelNode extends Object implements
  ArrayAccess,
  ITreeViewNode,
  IExpandableTreeViewNode //TODO ITree* nějak ošetřit dependencies
{
  public $data; //sql row
  ...
  {
    return PagesModel::getChildNodesWithMeta(
      $this->id,
      $this->lang,
      $key,
      $value
  ...
  public function getFiles($key = null)
  {
    ...
    if (isset($key)) {
      return isset($files[$key]) ? $files[$key] : new MissingFile();
    }
    return $files;
  }
  ...
  {
    $sqlwhere =
      array('id_page' => $this->id, 'deleted' => '0') + (array) $sqlwhere;
  ...
  public function getRedirectLink()
  {
    ...
    if (preg_match('~^#-p([0-9]+)-#$~', $pagelink, $m)) {
      return $presenter->link(':Front:Pages:', array(
        $m[1],
        'lang' => $this->lang
      ));
    }
  ...
  public function delete()
  {
    ...
    foreach (
      $this->getChildNodes()
      as $child //delete all childs recursively
    ) {
      $child->delete();
    }
  }
  ...
  public function save($newdata)
  {
    ...
    if (
      isset($newdata['name']) and
      $newdata['name'] == $this->data['heading']
    ) {
      $newdata['name'] = '';
    }

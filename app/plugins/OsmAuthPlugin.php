...
class OsmAuthPlugin extends Control
{
    ...
    {
        if (!$presenter->user->isInRole('user')) //probably admin -> allow
            return true;
        ...
        if (!in_array($presenter->view, explode(',', $presenters[$presenter->name]))) //not allowed view
            return false;
        ...
        if ($signal) {
            ...
            foreach ($signals[$presenter->name] as $s) {
                if (preg_match('~^' . $s . '$~', $signal)) //allowed signal
                    $allowed = true;
            }
    ...
    public static function allow_page_edit($presenter, $id)
    {
        ...
        if (count($presenter->request->post))
            $v = PagesModel::getPageById($id); //old
        else
            $v = $presenter->request;
        ...
        foreach (PagesModel::getPageById($id)->getParents() as $page) {
            if (in_array($page->id, $allowed)) //allowed to edit page?
                return true;
        }
